...
import WORKSTEP_FIELD from "@salesforce/schema/WorkStep.WorkOrderLineItemId";
import  WOLI_FIELD from "@salesforce/schema/WorkOrderLineItem.ParentWorkOrderLineItemId";
const FIELDSW = [WOLI_FIELD];
const FIELDSWS = [WORKSTEP_FIELD];
export default class ImageCapture extends LightningElement {
  ...
  @wire(getRecord, { recordId: "$recordId", fields: FIELDSW })
  WorkOrderLineItem;

  @wire(getRecord, { recordId: "$recordId", fields: FIELDSWS })
  WorkStep;
  ...
  async uploadAllPhotos() {
    ...
    if (this.objectApiName === 'WorkStep') {
   
      this.ParentId= getFieldValue(this.WorkStep.data, WORKSTEP_FIELD);
      console.log('********Parent*****Working on' +this.ParentId);
    }
   else  {
   
    this.ParentId= getFieldValue(this.WorkOrderLineItem.data, WOLI_FIELD);
    console.log('********Parent*****Working on' +this.ParentId);
   }
   if(this.ParentId == null) this.ParentId= this.recordId;
   console.log('*************Working on ' +this.ParentId);
